...
| Attribute | Total rows | OK | CONFORME | FUORI_ATTRIBUTO | ILLEGGIBILE | %FUORI overall |
|---|---:|---:|---:|---:|---:|---:|
| Aroma | 1 795 | 1 050 | 8 | 9 | 28 | 0.5 % |
| Sapore | 1 908 | 1 039 | 29 | 13 | 65 | 0.7 % |
| Spessore_della_Crosta | 1 789 | 1 046 | 4 | 46 | 77 | 2.6 % |
| Texture | 1 817 | 1 006 | 52 | 84 | 61 | 4.6 % |
| Struttura_della_Pasta | 2 133 | 1 138 | 23 | 144 | 54 | 6.7 % * |
| Profumo | 1 977 | 1 065 | 9 | 364 | 39 | 18.4 % |
| Colore_della_Pasta | 1 842 | 1 080 | 49 | 368 | 25 | 20.0 % |

\* On the trainable subset (`has_caption=True AND has_both_views=True`), *Struttura* reaches 47 %. *Spessore_della_Crosta* reaches 49 %. See §3.5.
...
The first Struttura pilot used the dataset loader without filtering on the `classe` column. Of the 1 071 trainable rows in Struttura, **502 (47 %) were marked FUORI_ATTRIBUTO** — these are captions that describe a *different* attribute and were intentionally not bound to Struttura by `12_riprocessa_fuori_attributo.py`. Examples:
...
When 47 % of the training distribution describes other attributes, the model learns a corrupted conditional and the test metrics — computed on the same noisy mixture — are a *lower bound* of the real architectural potential.
...
| Decoder | Family | Trainable params | Visual conditioning |
|---|---|---:|---|
| **LSTM scratch** | recurrent | ~ 20 M | visual tokens fed as `h0/c0` |
| **Transformer scratch** | attention | ~ 30 M | cross-attention over visual tokens |
| **GePpeTto pre-trained** | pre-trained transformer LM | ~ 124 M total (~ 0.4 M trainable when frozen) | prefix tuning: MLP 512→768, visual prefix concatenated to GPT-2 token embeddings |
...
Dataset (pre-fix): 1 071 train / 256 val / 265 test. FUORI_ATTRIBUTO contamination: 47 % in train, 49 % in test. Bug not yet known.

### 5.2 Bug discovery and second-pilot choice

End of April, while inspecting Struttura predictions qualitatively, we noticed systematic anomalies (predictions that talked about *texture* or *colour* under a Struttura attribute). Investigating the `classe` column revealed the loader bug. We computed `% FUORI_ATTRIBUTO` per attribute and chose **Sapore** (0.6 %, the cleanest after Aroma) as the natural counterfactual.
...
All 12 models beat all 4 baselines on every metric; the multiplier vs. retrieval baseline on BLEU-4 is 4× to 6×.
...
We computed bootstrap 95 % confidence intervals (n = 200 resamples with replacement, seed = 42) on the test predictions of the top 5 models.

![Figure 3 — Bootstrap 95 % CI for top 5 models on Sapore](figures/sapore_pilot/05_bootstrap_ci.png)

| Model | BLEU-4 (mean [95 % CI]) | METEOR (mean [95 % CI]) | ROUGE-L (mean [95 % CI]) |
|---|---|---|---|
| M3-FT | 0.118 [0.099, 0.138] | 0.305 [0.281, 0.328] | 0.263 [0.242, 0.288] |
| M5c | 0.111 [0.095, 0.129] | 0.320 [0.295, 0.345] | 0.261 [0.235, 0.289] |
| M5b-FT | 0.101 [0.080, 0.119] | 0.295 [0.273, 0.316] | 0.246 [0.222, 0.268] |
| M5b | 0.099 [0.080, 0.119] | 0.292 [0.269, 0.313] | 0.249 [0.224, 0.272] |
| M3 | 0.098 [0.079, 0.115] | 0.308 [0.285, 0.328] | 0.250 [0.230, 0.273] |

**The CIs of M3-FT and M5c overlap heavily on BLEU-4** (0.099–0.138 vs. 0.095–0.129). The point-estimate ranking favours M3-FT, but the difference is **not statistically significant at α = 0.05** with a single seed and a 230-sample test set. M5c's METEOR advantage (0.320 vs. 0.305) is also borderline. The honest reading is: *these three models perform similarly; the choice between them should be guided by other criteria (latency, output style, downstream usability)*.
...
On Sapore: M3 (0.098) > M1 (0.092); M3-FT (0.117) > M1-FT (0.098); M5c (0.112) > M5a (0.090). On Struttura the pattern was absent (all three encoders within 0.003 BLEU-4 of each other). Plausible explanation: ViT generates 196 spatial tokens per view, sufficient to capture global colour / texture / homogeneity correlations that predict taste; on the noisy Struttura dataset the noise floods every learnable visual pattern.
...
- **Latency** (single test caption, batch 1, RTX 4060): M1 ≈ 0.05 s, M3 ≈ 0.12 s, M5c ≈ 0.45 s (GePpeTto autoregressive decoding dominates).
- **VRAM at inference**: M1 ≈ 0.5 GB, M3 ≈ 1.2 GB, M5c ≈ 2.5 GB.
- **Disk footprint** of `best.pt`: M1 415 MB, M3 1.1 GB, M5c 1.9 GB — large because we save the full GPT-2 weights even when frozen. A future optimisation is to save only the projection MLP and reload GePpeTto from HuggingFace at inference.

For a deployment scenario showing a single caption per cheese sample to a human, **M5c on a 4 GB GPU is feasible** with sub-second latency. Batching helps amortise GPT-2 overhead.
...
python -m src.data.10_normalizza_commenti --attributo Sapore
python -m src.data.11_riprocessa_conforme --attributo Sapore
python -m src.data.12_riprocessa_fuori_attributo --attributo Sapore
python -m src.data.13_fix_hallucinations --attributo Sapore
python -m src.data.14_build_raw_per_attribute
python -m src.data.15_merge_captions_with_images
